...
create or replace rule ins_user as on insert to users_view
  do instead 
  insert into users(first_name,last_name,email,username,enc_pass,salt,dep,div)
  	values (new.first_name,new.last_name,new.email,new.username,make_pass(make_salt(new.password)||new.password),
  		make_salt(new.password),new.dep,new.div);
